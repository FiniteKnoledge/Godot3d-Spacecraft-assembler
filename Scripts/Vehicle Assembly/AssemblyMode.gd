...
@export var Parts:Array = ["res://PlacableParts/StructuralBlock2M.tscn"
,"res://PlacableParts/Thruster1.tscn"
,"res://PlacableParts/GyroBlock.tscn"]

@export var SelectedPart:int

var PartRotationOffset:Vector3
...
func _process(delta: float) -> void:
	...
	if !doohickey.button_pressed: #if assemblymode is on
		...
		if Input.is_action_just_pressed("ui_up"):
			PartRotationOffset.x += deg_to_rad(90)
		if Input.is_action_just_pressed("ui_down"):
			PartRotationOffset.x -= deg_to_rad(90)
		if Input.is_action_just_pressed("ui_right"):
			PartRotationOffset.z -= deg_to_rad(90)
...
func _physics_process(delta: float) -> void:
	...
	if result:
		...
		if result.collider is RigidBody3D and result.collider:
			#Preview
			placement_preview.show()
			placement_preview.position = result.collider.position+result.normal*2
			placement_preview.rotation = result.collider.rotation+PartRotationOffset
			if Input.is_action_just_pressed("click"): #On click
				...
				NewPartReady.rotation = result.collider.rotation+PartRotationOffset
...
func create_joint(body_a: RigidBody3D, body_b: RigidBody3D, joint_position: Vector3):
	var joint = load("res://PlacableParts/Joint.tscn")
	var jointMade = joint.instantiate()
	vehicle.add_child(jointMade)
	jointMade.global_position = joint_position
	
	jointMade.node_a = body_a.get_path()
	jointMade.node_b = body_b.get_path()
	#joint.
